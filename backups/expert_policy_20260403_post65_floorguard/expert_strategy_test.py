...
class ExpertPolicy:
    ...
    def get_actions(self, step: int,
        ...
        for i in range(n):
            ...
            goal_guard_mode = goal_dist < 1.35 and min_target_dist > (self.cfg.catch_radius + 0.08)
            if goal_guard_mode and anchor_idx != 0:
                ...
                waypoint = target_pos_pred + 0.26 * forward_dir + side_axis * 0.24
            ...
            floor_guard = drone_pos[i, 2] < 1.55 or (drone_pos[i, 2] < 1.85 and drone_vel[i, 2] < -0.20)
            if floor_guard:
                v_des[2] = max(float(v_des[2].item()), 0.85)
                if anchor_idx != 0:
                    speed_cap = min(speed_cap, 0.90 * self.cfg.v_drone)
