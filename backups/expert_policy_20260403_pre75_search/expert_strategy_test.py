...
class ExpertPolicy:
    ...
    def _build_formation_targets(
        ...
        lookahead_time = 0.20 if goal_dist > 1.4 else 0.12
        ...
        forward_dir = safe_normalize((0.78 * goal_dir + 0.22 * motion_dir).unsqueeze(0)).squeeze(0)
    ...
    def get_actions(self, step: int,
        ...
        target_dists = torch.norm(drone_pos - target_pos_flat, dim=-1)
        min_target_dist = target_dists.min().item()
        closest_idx = int(target_dists.argmin().item())
        goal_dist = torch.norm(goal_pos - target_pos_flat).item()
        late_emergency = (
            step > int(0.55 * self.cfg.max_episode_length)
            and min_target_dist < 0.85
            and min_target_dist > (self.cfg.catch_radius + 0.06)
        )
        ...
        for i in range(n):
            ...
            close_mode = dist_wp < 0.45 or (i == closest_idx and min_target_dist < 1.00) or late_emergency
            ...
            if close_mode:
                if i == closest_idx:
                    close_wp = target_pos_pred - 0.01 * forward_dir
                    inward_gain = 0.22
                    strike_gain = 2.18
                    damping = 0.38
                elif anchor_idx == 0:
                    ...
                    close_wp = target_pos_pred - 0.01 * forward_dir + side_axis * (0.14 if trap_mode else 0.20)
                    inward_gain = 0.08
                    strike_gain = 1.92
                    damping = 0.32
                if late_emergency:
                    if i == closest_idx:
                        close_wp = target_pos_pred - 0.02 * forward_dir
                        inward_gain = max(inward_gain, 0.24)
                        strike_gain = max(strike_gain, 2.22)
                        damping = max(damping, 0.38)
                    elif anchor_idx == 0:
                        close_wp = target_pos_pred + 0.02 * forward_dir
                        inward_gain = max(inward_gain, 0.18)
                        strike_gain = max(strike_gain, 2.06)
                        damping = max(damping, 0.36)
                    else:
                        side_axis = project_to_plane(waypoint - target_pos_flat, forward_dir)
                        if side_axis.norm() > 1e-5:
                            side_axis = safe_normalize(side_axis.unsqueeze(0)).squeeze(0)
                            ...
                            side_axis = torch.zeros_like(forward_dir)
                        close_wp = target_pos_pred - 0.05 * forward_dir + side_axis * (0.10 if trap_mode else 0.16)
                        inward_gain = max(inward_gain, 0.14)
                        strike_gain = max(strike_gain, 2.02)
                        damping = max(damping, 0.34)
                inward = safe_normalize((target_pos_pred - drone_pos[i]).unsqueeze(0)).squeeze(0)
                v_des = (
                    strike_gain * (close_wp - drone_pos[i])
                    + 0.94 * target_vel_ff
                    + inward_gain * inward * self.cfg.v_drone
                    - damping * drone_vel[i]
                )
                capture_height = float(target_pos_pred[2].item()) + (0.06 if i == closest_idx else 0.10)
                if late_emergency:
                    capture_height += 0.02
                v_des[2] = 0.92 * (capture_height - drone_pos[i, 2]) - 0.50 * drone_vel[i, 2]

            speed_cap = self.cfg.v_drone if (not close_mode or i == closest_idx or anchor_idx == 0 or late_emergency) else 0.96 * self.cfg.v_drone
            floor_guard = drone_pos[i, 2] < 1.50 or (drone_pos[i, 2] < 1.80 and drone_vel[i, 2] < -0.20)
            if floor_guard:
                v_des[2] = max(float(v_des[2].item()), 0.55)
                if i != closest_idx and anchor_idx != 0:
                    speed_cap = min(speed_cap, 0.94 * self.cfg.v_drone)
